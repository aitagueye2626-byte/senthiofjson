  ...
  return `
    <div id="dashboardAdmin">
      <nav class="barre-navigation">
        <div class="logo-barre"><span>🐟</span><span>SenThiof-JSON - Admin</span></div>
...
// getStocks est défini dans data.js
...
function getSectionStock(produits, transactions, stocks) {
  ...
  return `
    ...
    <div class="section-tableau"><div class="table-container"><table id="tableauStock">
...
function mettreAJourStock(nom, delta) {
  const stocks = JSON.parse(localStorage.getItem("senthiof_stocks") || "[]");
  const idx = stocks.findIndex(s => s.nom.toLowerCase() === nom.toLowerCase());
  if (idx !== -1) {
    stocks[idx].disponible = Math.max(0, stocks[idx].disponible + delta);
    localStorage.setItem("senthiof_stocks", JSON.stringify(stocks));
  } else {
    stocks.push({ id: Date.now(), nom, disponible: Math.max(0, delta) });
    localStorage.setItem("senthiof_stocks", JSON.stringify(stocks));
  }
...
function initialiserStatuts(transactions) {
  const mises = transactions.filter(t => !t.statut).map(t =>
    modifierTransaction(t.id, { ...t, statut: "Livrée" })
  );
  return Promise.all(mises);
}
...
function getSectionCommandes() {
  return `
    ...
    <div class="section-tableau"><div class="table-container"><table><thead><tr><th>Clients</th><th>Montants</th><th>Statuts</th><th>Action</th></tr></thead><tbody id="corpsCommandes"></tbody></table>
...
function changerStatutCommande(client) {
  getTransactions().then(transactions => {
    ...
    Promise.all(siennes.map(t => modifierTransaction(t.id, { ...t, statut: nouveauStatut })
